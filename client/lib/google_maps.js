gmaps = {
  ...
  initialize: function() {
    ...
    setInterval(function() {
      navigator.geolocation.getCurrentPosition(function(position) {
        ...
        if (Session.get('testMode') == true) {
          lat = Session.get('startLat') - .00003;
          lng = Session.get('startLng') + .00005;
    ...
    for (var i = 0; i < friendList.findOne({
        userId: Meteor.userId()
      }).friends.length; i++) {
      friendsId = friendList.findOne({
        userId: Meteor.userId()
      }).friends[i];
      friendsPositionLat = Markers.findOne({
        userId: friendsId
      }).positionLat;
      friendsPositionLon = Markers.findOne({
        userId: friendsId
      }).positionLon;
      friendsPicture = Markers.findOne({
        userId: friendsId
      }).img;
      sms = Meteor.users.findOne({
        _id: friendsId
      }).profile.smsAddress;
      icon = {
